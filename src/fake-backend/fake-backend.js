...
export function fakeAPIFetch(options) {
  ...
  } else if (options.url.includes("spacecraft")) {
	return handleFakeRequestForSpacecraft(options.url);
  }
...
function handleFakeRequestForSpacecraft(url) {
  if (url.includes("page")) {
    return handleListRequest(url, spacecraft, modifySpacecraft);
  } else {
    return handleIndividualRequest(url, spacecraft, modifySpacecraft);
  }
}
...
function modifySpacecraft(spacecraft) {
  const films = getFilmsThatMatchId(spacecraft.id, "spacecraft");
  return {
    ...spacecraft,
    films,
  };
